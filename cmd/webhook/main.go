...
func main() {
	...
	sharedmain.MainWithConfig(webhook.WithOptions(injection.WithNamespaceScope(signals.NewContext(), system.Namespace()), webhook.Options{
		Port:        options.Port,
		ServiceName: "etcd-operator-webhook",
		SecretName:  "etcd-operator-webhook-cert",
	}), "webhook", injection.ParseAndGetRESTConfigOrDie(),
...
func newCRDDefaultingWebhook(ctx context.Context, w configmap.Watcher) *controller.Impl {
	return defaulting.NewAdmissionController(ctx,
		"defaulting.webhook.provisioners.etcd-operator.sh",
		"/default-resource",
		apis.Resources,
		InjectContext,
		true,
...
func newCRDValidationWebhook(ctx context.Context, w configmap.Watcher) *controller.Impl {
	return validation.NewAdmissionController(ctx,
		"validation.webhook.provisioners.etcd-operator.sh",
		"/validate-resource",
		apis.Resources,
		InjectContext,
		true,
...
func newConfigValidationController(ctx context.Context, cmw configmap.Watcher) *controller.Impl {
	return configmaps.NewAdmissionController(ctx,
		"validation.webhook.config.etcd-operator.sh",
		"/config-validation",
